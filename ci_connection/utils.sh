...
_normalize_path() {
  if (( _IS_WINDOWS )); then cygpath -m "$1"
  else echo "$1"
  fi
}
...
download_and_verify_uv() {
  ...
  while ! curl --proto '=https' --tlsv1.2 -sSfL "${url}" -o "${tmp_file}"; do
    (( attempt++ ))
    (( attempt > max_attempts )) && { echo "ERR: Download failed after ${max_attempts} attempts" >&2; return 1; }
...
unpack_and_setup_uv() {
  local archive_path="$1" target_triple="$2" uv_bin_dir="$HOME/.uv/bin" env_script_path="$HOME/.uv/env"
  mkdir -p "$uv_bin_dir"
  local unpack_dir
  unpack_dir=$(mktemp -d)
  trap 'rm -rf "${unpack_dir}"' EXIT RETURN

  local extracted_uv_content_dir="${unpack_dir}/uv-${target_triple}"
  ...
  local uv_exe_name="uv" uvx_exe_name="uvx"
  if (( _IS_WINDOWS )); then
    uv_exe_name="uv.exe"; uvx_exe_name="uvx.exe"
  fi
  ...
  local norm_uv_bin_dir="$(_normalize_path "$uv_bin_dir")"
  cat > "$env_script_path" << ENV_SCRIPT
#!/bin/sh
case ":\${PATH}:" in *:'${norm_uv_bin_dir}':*) ;; *) export PATH="${norm_uv_bin_dir}:\${PATH}" ;; esac
ENV_SCRIPT
  chmod +x "$env_script_path"

  local profile_files=("$HOME/.profile" "$HOME/.bash_profile" "$HOME/.bashrc" "$HOME/.zshrc")
  local source_line=". '$(_normalize_path "$env_script_path")'"
...
suitable_python_exists() {
  ...
  if [[ "$major" -lt "$MIN_PYTHON_MAJOR_VERSION" ]] || { [[ "$major" -eq 3 && "$minor" -lt "$MIN_PYTHON_MINOR_VERSION" ]]; }; then
    return 1
  fi
  ...
  echo "$(_normalize_path "$py_exe")"
}

ensure_suitable_python_is_available() {
  local -n python_bin_path="$1"
  ...
  if ! command -v uv &>/dev/null; then
    if (( _IS_WINDOWS )); then
      tmp_archive=$(mktemp "uv-download-XXXXXX.zip") || { echo "ERR: mktemp failed"; return 1; }
    else
      tmp_archive=$(mktemp "uv-download-XXXXXX.tar.gz") || { echo "ERR: mktemp failed"; return 1; }
    fi
  ...
  uv python install "$UV_PYTHON_TO_INSTALL"
  python_bin_path="$(uv python find "$UV_PYTHON_TO_INSTALL")"
}
